...
# filter down to grants
revenue_grants <- filter(Revenue_data, funding_class%in%c("STATE", "FEDERAL - OTHER"))%>%
...
revenue_grants1 <- filter(Revenue_data, revenue_category%in%c("Federal Grants-Categorical", 
...
revenue_grants_prefiltered <- filter(Revenue_data, revenue_category%in%c("Federal Grants-Categorical", 
                                                              "Unrestricted Federal and State Aid",
                                                              "State Grants and Contracts-Categorical",
                                                              "State Grants-Categorical",
                                                              "Federal Grants and Contracts-Categorical",
...
revenue_grants_prefiltered_sum_by_year_cat <- group_by(revenue_grants_prefiltered,budget_fiscal_year, closing_classification_name) %>% 
  summarise(sum=sum(recognized)) %>% 
  mutate(sum=round(sum/1000000))


revenue_grants_prefiltered_sum_by_year <- group_by(revenue_grants_prefiltered,budget_fiscal_year) %>% 
  summarise(sum=sum(recognized)) %>% 
  arrange(desc(budget_fiscal_year)) %>% 
  mutate(sum=sum/1000)


revenue_grants_prefiltered_sum_by_year$truth <- truth_df$amount[1:13]
revenue_grants_prefiltered_sum_by_year$diff <- round((revenue_grants_prefiltered_sum_by_year$truth-revenue_grants_prefiltered_sum_by_year$sum)/1000)

revenue_unrestricted <- filter(Revenue_data, revenue_category=="Unrestricted Federal and State Aid")


revenue_grants_drop_contracts <- filter(Revenue_data, revenue_category%in%c("Federal Grants-Categorical", 
                                                              "Unrestricted Federal and State Aid",
                                                              "State Grants-Categorical",
                                                              "Non-Governmental Grants"))%>%
  filter(closing_classification_name != "Collected Unearned Revenue Roll" &
...
revenue_grants_sum_by_year <- group_by(revenue_grants,budget_fiscal_year) %>% 
  summarise(sum=sum(recognized)) %>% 
  arrange(desc(budget_fiscal_year)) %>% 
  mutate(sum=sum/1000)

revenue_grants_sum_by_year1 <- group_by(revenue_grants1,budget_fiscal_year) %>% 
  summarise(sum=sum(recognized)) %>% 
  arrange(desc(budget_fiscal_year)) %>% 
  mutate(sum=sum/1000)

revenue_grants_sum_by_year_drop_contracts <- group_by(revenue_grants_drop_contracts,budget_fiscal_year) %>% 
  summarise(sum=sum(recognized))


revenue_grants_sum_by_year_cat <- group_by(revenue_grants,fiscal_year, revenue_category) %>% 
  summarise(sum=sum(recognized))
revenue_grants_sum_by_year_cat1 <- group_by(revenue_grants1,fiscal_year, closing_classification_name) %>% 
  summarise(sum=sum(recognized)) %>% 
  mutate(sum=round(sum/1000000))

revenue_grants_sum_by_year$truth <- truth_df$amount[1:13]
revenue_grants_sum_by_year$diff <- round((revenue_grants_sum_by_year$truth-revenue_grants_sum_by_year$sum)/1000)

revenue_grants_sum_by_year1$truth <- truth_df$amount[1:13]
revenue_grants_sum_by_year1$diff <- round((revenue_grants_sum_by_year1$truth-revenue_grants_sum_by_year1$sum)/1000)
